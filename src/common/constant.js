export const ENDPOINT = '/api/v1';
export const ADMIN_PRODUCTION_ENDPOINT = '/admin/productions';
export const ADMIN_ALBUM_ENDPOINT = '/admin/albums';
export const ADMIN_CATEGORIES_ENDPOINT = '/admin/categories';
export const ADMIN_ARTIST_ENDPOINT  = '/admin/artists';
export const ADMIN_SONG_ENDPOINT  = '/admin/songs';
export const ADMIN_ARTIST_SONG_ENDPOINT = '/admin/artist_songs';